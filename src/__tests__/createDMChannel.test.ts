...
test('createDMChannel(): returns channel if user ID is valid', async () => {
	for (let i = 0; i < users.length; i++) {
		mock.onPost(`unics_social/users/${users[i].id}/channel`).reply(200, { channel: channels[i] });
		const response = await myAPIClient.createDMChannel(users[i].id);
		expect(response).toEqual(channels[i]);
	}
});

test('createDMChannel(): throws error if user ID is invalid', async () => {
	for (const user of users) {
		mock.onPost(`unics_social/users/${user.id}/channel`).reply(400, { error: 'invalid user ID' });
		await expect(unwrapErrorBody(myAPIClient.createDMChannel(user.id))).rejects.toMatchObject({ error: 'invalid user ID' });
	}
});
